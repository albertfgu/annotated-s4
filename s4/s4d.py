...
# ### Scratch
# Karan feedback:
# The only thing missing seems to be some reflection and takeaways at the end, since the post is quite long.
# Some elements are actually in there (e.g. open problems), but it might be good to pull them together
#
# - what version should someone wanting to use S4 use, when does it make sense to use the full DPLR representation? This is easier to talk about now that you have the results in there.
#
# - what problems would we like people to work on, this is a nice opportunity to make that really concrete (outside of finding non-HiPPO inits)?
#
# - any other references or pointers for discussion of other aspects of SSMs e.g. the other HTTYH work, new LRA # for S4 etc.
#
# - reference to the state-spaces repo for the official version of code
