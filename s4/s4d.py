...
# Finally, materializing the full Vandermonde matrix is unnecessary and can be optimized away to save a lot of memory! We elaborate on this below. [**Link**]
...
# As the original S4 paper specified, this S4D kernel is just a single Vandermonde matrix-vector product. Just as with all SSMs, we can test that convolving by this kernel produces the same answer as the sequential scan.
...
# we're ready to put the S4D layer together!
...
# The core of the S4D layer is the same as the traditional SSM layer defined in the first part of the post. We define our learnable weights $C$ then call the kernel code written above as a convolution during training.
# Finally, during discrete decoding, we use the initial recurrence computed above.

# ... and that's all folks! S4D is dramatically more easy to understand and compact (50 LoC!) than S4, with an extremely structured state space that reduces to a single Vandermonde product. Together with the new theoretical insights in the next section, we can build a model that is almost as expressive and performant as S4.
...
# This is the only part of this post that requires some mathematical background, but is optional: the entire algorithm is already fully contained in Parts I and II.

# The initialization is given by the line `hippo_A_initializer`, which is the diagonal part of the DPLR representation of S4's HiPPO matrix.
...
# iit's actually extremely difficult to find a diagonal $\boldsymbol{A}$ that performs well!
...
# To circumvent this, S4 discovered a new way to put a matrix in *almost diagonal* form, while only needing to conjugate by *unitary matrices* which are perfectly stable.
#
# $$
# \begin{aligned}
...
# As discussed in Part 1 of the Annotated S4 [Link], the first component $\boldsymbol{A}^{(N)}$ is a *normal matrix* which is unitarily diagonalizable, hence $\boldsymbol{A}$ is unitarily equivalent to a DPLR matrix.
...
# The $N$ different scalar equations can be vectorized to just say that
# $$
...
# Let's plot $e^{t \bm{A}^{(N)}} \frac{\bm{B}}{2}$ for $N=256$ (*Left*) and $N=1024$ (*Right*):
...
# Inspired by the HiPPO-diagonal matrix, S4D proposes several new initializations with simpler formulas where the real part is fixed to $-\frac{1}{2}$ and the imaginary part follows a polynomial law.
...
# The following table shows results for several variants of S4 and S4D with various $(\boldsymbol{A}, \boldsymbol{B})$ initializations, using a slightly improved architecture from the original S4 paper.
...
# However, the original S4 initialization and its diagonal approximation are thus far the only ones that can solve Path-X.
...
# However, there's much left to understand here, and we believe that perhaps even simpler and better models may be uncovered!
